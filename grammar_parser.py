...
def get_token_pos(token):
    # Check which category the token belongs to in word_dict
    for pos, words in word_dict.items():
        if token in words:
            return pos  # Return the category (e.g., 'nouns', 'verbs', etc.)
...
def parse_sentence(sentence):
    ...
    token_with_pos = {token: get_token_pos(token) for token in tokens}  # Map tokens to their POS

    try:
        parsed_trees = list(parser.parse(tokens))
        if parsed_trees:
            return {"is_valid": True, "tree": parsed_trees[0], "tokens": token_with_pos}
        else:
            error_word = find_error_word(tokens, parser)
            return {
                "is_valid": False,
                "error": f"Invalid structure near the word '{error_word}'.",
                "tokens": token_with_pos
            }
    except ValueError as e:
        return {"is_valid": False, "error": f"Grammar error: {str(e)}", "tokens": token_with_pos}
